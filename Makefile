...
.PHONY: ko
ko: $(KO) ## Download kustomize locally if necessary. If wrong version is installed, it will be removed before downloading.
$(KO): $(LOCALBIN)
...
deploy-kind:
	...
	make install
	make build-image # TODO: how do you kind.local here ?
	kind load docker-image ${KO_DOCKER_REPO}:${RELEASE_VERSION} --name ${KIND_CLUSTER_NAME}
	helm template kro ./helm --namespace kro-system --set image.pullPolicy=Never --set image.repository=${KO_DOCKER_REPO} --set image.tag=${RELEASE_VERSION} | $(KUBECTL) apply -f -
	sleep 5
