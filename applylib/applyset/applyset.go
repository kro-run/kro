...
func (a *applySet) updateParentLabelsAndAnnotations(ctx context.Context, mode ApplySetUpdateMode) error {
	...
	jsonPatch, err := parentPatch.MarshalJSON()
	if err != nil {
		return fmt.Errorf("error marshalling parent patch when updating applyset labels and annotations: %w", err)
	}
	...
	if !reflect.DeepEqual(original.GetLabels(), parentPatch.GetLabels()) ||
		...
		if _, err := a.parentClient.Patch(ctx,
			a.parent.GetName(),
			types.ApplyPatchType,
			jsonPatch,
			metav1.PatchOptions{FieldManager: "kro-controller"},
		); err != nil {
